...
$('#specimen').innerHTML=specimenOptions;
...
function syncControls(){const video=isVideoSpecimen(),total=video?'依影片':state.objective*state.eyepiece+'×';$('#totalMag').textContent=total;$('#viewTotalMag').textContent=video?'影片倍率':total;$('#specimen').value=state.specimen;$('#viewSpecimen').value=state.specimen;['coarse','fine','aperture','mirror'].forEach(id=>{const main=$('#'+id),view=$('#view'+id[0].toUpperCase()+id.slice(1));if(main)main.value=state[id];if(view)view.value=state[id];if(main)main.disabled=video;if(view)view.disabled=video});$('#coarseOut').textContent=video?'鎖定':state.coarse;$('#fineOut').textContent=video?'鎖定':state.fine;$('#apertureOut').textContent=video?'鎖定':state.aperture+'%';$('#mirrorOut').textContent=video?'鎖定':state.mirror;$('#viewCoarseOut').textContent=video?'影片':state.coarse;$('#viewFineOut').textContent=video?'影片':state.fine;$('#viewApertureOut').textContent=video?'影片':state.aperture+'%';$('#viewMirrorOut').textContent=video?'影片':state.mirror;$$('.objective').forEach(b=>b.classList.toggle('active',+b.dataset.mag===state.objective));$$('.view-objective').forEach(b=>b.classList.toggle('active',+b.dataset.viewObjective===state.objective));$$('.view-objective').forEach(b=>b.disabled=video);}
...
$('#specimen').onchange=e=>setSpecimen(e.target.value);$('#viewSpecimen').onchange=e=>setSpecimen(e.target.value);$('#eyepiece').onchange=e=>{state.eyepiece=+e.target.value;update()};
