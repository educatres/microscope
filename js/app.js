...
const camera=new THREE.PerspectiveCamera(42,$('#scene').clientWidth/$('#scene').clientHeight,.1,100); camera.position.set(7,5.3,8.5);
const renderer=new THREE.WebGLRenderer({antialias:true}); renderer.setPixelRatio(Math.min(devicePixelRatio,2)); renderer.setSize($('#scene').clientWidth,$('#scene').clientHeight); renderer.shadowMap.enabled=true; $('#scene').appendChild(renderer.domElement);
...
addEventListener('resize',()=>{const el=$('#scene');camera.aspect=el.clientWidth/el.clientHeight;camera.updateProjectionMatrix();renderer.setSize(el.clientWidth,el.clientHeight)});
...
function update(){const total=state.objective*state.eyepiece;$('#totalMag').textContent=total+'×';$('#coarseOut').textContent=state.coarse;$('#fineOut').textContent=state.fine;$('#apertureOut').textContent=state.aperture+'%';$('#mirrorOut').textContent=state.mirror;mirror.rotation.z=(state.mirror-50)*.012;objectives.forEach((o,i)=>o.material.emissive?.setHex(Number(o.userData.name.startsWith(state.objective))?0x332500:0));updateFeedback();}
...
function showView(){state.viewed=true;const sp=specimens[state.specimen],total=state.objective*state.eyepiece,err=focusError(),b=brightness();const view=$('#microscopeView');view.style.backgroundImage=`url("${sp.url}")`;view.style.backgroundSize=`${110+state.objective*5}%`;view.style.backgroundPosition=`${50-state.x*35}% ${50+state.y*35}%`;view.style.filter=`brightness(${b}) contrast(${.75+state.aperture/160}) blur(${Math.min(14,err/6)}px) saturate(${.8+b*.35})`;$('#viewTitle').textContent=sp.name;$('#viewInfo').textContent=`${total}×｜亮度 ${b<.4?'不足':b>1.05?'過強':'適中'}｜${err<12?'清晰':err<35?'接近焦點':'模糊'}`;$('#scaleBar b').textContent=state.objective===4?'500 µm':state.objective===10?'200 µm':'50 µm';$('#scaleBar span').style.width=state.objective===4?'110px':state.objective===10?'80px':'55px';$('#viewerModal').classList.remove('hidden');}
...
$$('.objective').forEach(b=>b.onclick=()=>{$$('.objective').forEach(x=>x.classList.remove('active'));b.classList.add('active');state.objective=+b.dataset.mag;update()});
['coarse','fine','aperture','mirror'].forEach(id=>$('#'+id).oninput=e=>{state[id]=+e.target.value;update()});
...
$$('[data-move]').forEach(b=>b.onclick=()=>{const d=b.dataset.move,step=.12;if(d==='up')state.y-=step;if(d==='down')state.y+=step;if(d==='left')state.x+=step;if(d==='right')state.x-=step;if(d==='center')state.x=state.y=0;state.x=Math.max(-1,Math.min(1,state.x));state.y=Math.max(-1,Math.min(1,state.y));update()});
