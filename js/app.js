...
$('#specimen').innerHTML=Object.entries(specimens).map(([key,sp])=>`<option value="${key}">${sp.name}</option>`).join('');
...
function syncControls(){const video=isVideoSpecimen(),total=video?'依影片':state.objective*state.eyepiece+'×';$('#totalMag').textContent=total;$('#viewTotalMag').textContent=video?'影片倍率':total;['coarse','fine','aperture','mirror'].forEach(id=>{const main=$('#'+id),view=$('#view'+id[0].toUpperCase()+id.slice(1));if(main)main.value=state[id];if(view)view.value=state[id]});['coarse','fine'].forEach(id=>{const main=$('#'+id),view=$('#view'+id[0].toUpperCase()+id.slice(1));if(main)main.disabled=video;if(view)view.disabled=video});$('#coarseOut').textContent=video?'鎖定':state.coarse;$('#fineOut').textContent=video?'鎖定':state.fine;$('#apertureOut').textContent=state.aperture+'%';$('#mirrorOut').textContent=state.mirror;$('#viewCoarseOut').textContent=video?'影片':state.coarse;$('#viewFineOut').textContent=video?'影片':state.fine;$('#viewApertureOut').textContent=state.aperture+'%';$('#viewMirrorOut').textContent=state.mirror;$$('.objective').forEach(b=>b.classList.toggle('active',+b.dataset.mag===state.objective));$$('.view-objective').forEach(b=>b.classList.toggle('active',+b.dataset.viewObjective===state.objective));$$('.view-objective').forEach(b=>b.disabled=video);}
function update(){syncControls();if(!$('#viewerModal').classList.contains('hidden'))renderView();updateFeedback();}
function updateFeedback(){if(isVideoSpecimen()){const centered=Math.hypot(state.x,state.y)<.25;$('#feedback').textContent=centered?'影片模式：倍率依影片為主，粗調與細調已鎖定，可移動載玻片觀察不同位置。':'影片模式：倍率依影片為主，可用載玻片方向鍵移動視野。';return}const err=focusError(),b=brightness();let t='';if(state.objective===40&&Math.abs(state.coarse-specimens[state.specimen].ideal.c)>5)t='高倍下不應大幅使用粗調節輪，以免物鏡碰撞玻片。';else if(b<.4)t='視野太暗：調整反光鏡或開大光圈。';else if(b>1.05)t='光線過強：縮小光圈可提高對比。';else if(err>35)t='影像非常模糊，低倍時先用粗調節輪找像。';else if(err>12)t='已看到標本，再用細調節輪讓邊緣清晰。';else if(Math.hypot(state.x,state.y)>.55)t='影像清楚，但標本未置中；移動載玻片。';else t='影像清楚且置中，可以觀察或切換更高倍率。';$('#feedback').textContent=t;}
function renderView(){const sp=specimens[state.specimen],video=isVideoSpecimen(),total=state.objective*state.eyepiece,err=focusError(),b=brightness(),view=$('#microscopeView');let frame=$('#videoView');if(video){view.style.backgroundImage='none';view.style.filter='none';view.style.backgroundPosition='center';view.style.backgroundSize='cover';if(!frame){frame=document.createElement('iframe');frame.id='videoView';frame.className='video-view';frame.allow='accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share';frame.allowFullscreen=true;view.prepend(frame)}const src=`https://www.youtube.com/embed/${sp.youtube}?start=${sp.start}&autoplay=1&rel=0&playsinline=1`;if(frame.dataset.src!==src){frame.src=src;frame.dataset.src=src}frame.style.transform=`translate(${state.x*16}%,${state.y*16}%) scale(1.18)`;$('#viewTitle').textContent=sp.name;$('#viewInfo').textContent=`倍率依影片為主｜從 ${sp.start} 秒開始播放｜粗調、細調鎖定`;$('#scaleBar b').textContent='影片倍率';$('#scaleBar span').style.width='72px';syncControls();return}if(frame){frame.remove()}view.style.backgroundImage=`url("${sp.url}")`;view.style.backgroundSize=`${110+state.objective*5}%`;view.style.backgroundPosition=`${50-state.x*35}% ${50+state.y*35}%`;view.style.filter=`brightness(${b}) contrast(${.75+state.aperture/160}) blur(${Math.min(14,err/6)}px) saturate(${.8+b*.35})`;$('#viewTitle').textContent=sp.name;$('#viewInfo').textContent=`${total}×｜亮度 ${b<.4?'不足':b>1.05?'過強':'適中'}｜${err<12?'清晰':err<35?'接近焦點':'模糊'}`;$('#scaleBar b').textContent=state.objective===4?'500 µm':state.objective===10?'200 µm':'50 µm';$('#scaleBar span').style.width=state.objective===4?'110px':state.objective===10?'80px':'55px';syncControls();}
