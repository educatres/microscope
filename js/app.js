...
const state={specimen:'paramecium400',objective:4,eyepiece:10,coarse:0,fine:0,aperture:65,bulbBrightness:60,x:0,y:0,viewed:false};
const specimens={
 kalanchoe:{name:'Kalanchoe 葉片下表皮',url:'https://upload.wikimedia.org/wikipedia/commons/9/9a/Kalanchoe_leaf.jpg',ideal:{c:54,f:56},magnification:400,desc:'長壽花葉片的下表皮，可觀察表皮細胞與氣孔。'},
 tilia:{name:'Tilia 椴樹下表皮',url:'https://commons.wikimedia.org/wiki/Special:FilePath/Plant%20leaf%20epidermis%20%28255%2017%29%20Lower%20epidermis%20of%20lime%20tree%20%28Tilia%29.jpg?width=1600',ideal:{c:52,f:54},magnification:600,desc:'椴樹葉片下表皮，影像原始倍率 600×。'},
 cheekIodine:{name:'碘液染色口腔細胞',url:'https://commons.wikimedia.org/wiki/Special:FilePath/Human%20Cheek%20Cells%20%28Iodine%20Stain%29.jpg?width=1600',ideal:{c:57,f:54},magnification:400,desc:'碘液染色的人類口腔上皮細胞，可觀察細胞核。'},
 paramecium400:{name:'草履蟲 Paramecium',url:'https://commons.wikimedia.org/wiki/Special:FilePath/Paramecium_400x.jpg?width=1600',ideal:{c:56,f:52},magnification:400,desc:'草履蟲，體表密布纖毛。'},
 euglena60:{name:'眼蟲 Euglena',url:'https://commons.wikimedia.org/wiki/Special:FilePath/20100306_204728_Euglena.jpg?width=1600',ideal:{c:47,f:56},magnification:900,defaultObjective:60,defaultEyepiece:15,desc:'眼蟲，使用 60× 物鏡與 15× 目鏡觀察。'},
...
const specimenOptions=Object.entries(specimens).map(([key,sp])=>`<option value="${key}">${sp.name}${sp.magnification?`｜${sp.magnification}×`:''}</option>`).join('');
...
function brightness(){return Math.max(.08,Math.min(1.5,(state.aperture/70)*(state.bulbBrightness/60)/(1+Math.log10(state.objective)*.18)))}
function syncControls(){const video=isVideoSpecimen(),total=video?'依影片':state.objective*state.eyepiece+'×';$('#viewTotalMag').textContent=video?'影片倍率':total;$('#viewSpecimen').value=state.specimen;$('#eyepiece').value=state.eyepiece;$('#eyepiece').disabled=video;['coarse','fine','aperture','bulbBrightness'].forEach(id=>{const view=$('#view'+id[0].toUpperCase()+id.slice(1));view.value=state[id];view.disabled=video});$('#viewCoarseOut').textContent=video?'影片':state.coarse;$('#viewFineOut').textContent=video?'影片':state.fine;$('#viewApertureOut').textContent=video?'影片':state.aperture+'%';$('#viewBulbBrightnessOut').textContent=video?'影片':state.bulbBrightness+'%';$$('.view-objective').forEach(b=>{b.classList.toggle('active',+b.dataset.viewObjective===state.objective);b.disabled=video});}
...
function updateFeedback(){if(isVideoSpecimen()){const centered=Math.hypot(state.x,state.y)<.25;$('#feedback').textContent=centered?'影片模式：倍率依影片為主，粗調、細調、光圈與燈泡亮度已鎖定，可移動載玻片觀察不同位置。':'影片模式：倍率依影片為主，可用載玻片方向鍵移動視野。';return}const err=focusError(),b=brightness();let t='';if(state.objective>=40&&Math.abs(state.coarse-specimens[state.specimen].ideal.c)>5)t='高倍下不應大幅使用粗調節輪，以免物鏡碰撞玻片。';else if(b<.4)t='視野太暗：提高燈泡亮度或開大光圈。';else if(b>1.05)t='光線過強：降低燈泡亮度或縮小光圈。';else if(err>35)t='影像非常模糊，低倍時先用粗調節輪找像。';else if(err>12)t='已看到標本，再用細調節輪讓邊緣清晰。';else if(Math.hypot(state.x,state.y)>.55)t='影像清楚，但標本未置中；移動載玻片。';else t='影像清楚且置中，可以觀察或切換更高倍率。';$('#feedback').textContent=t;}
...
function dockViewer(mode){const shell=$('.viewer-shell'),controls=$('.viewer-controls'),inline=mode==='inline';if(inline){$('#inlineViewerHost').append(shell);$('#viewerControlsHost').append(controls);$('#inlineViewerHost').classList.remove('hidden');$('#viewerModal').classList.add('hidden')}else{$('#viewerModal').append(shell);$('.viewer-layout').append(controls);$('#inlineViewerHost').classList.add('hidden');}shell.classList.toggle('inline-viewer-shell',inline);$('#closeViewer').classList.toggle('hidden',inline);}
...
['coarse','fine','aperture','bulbBrightness'].forEach(id=>{$('#view'+id[0].toUpperCase()+id.slice(1)).oninput=e=>setRange(id,e.target.value)});
function setSpecimen(value){state.specimen=value;const sp=specimens[value];if(sp.defaultObjective)state.objective=sp.defaultObjective;if(sp.defaultEyepiece)state.eyepiece=sp.defaultEyepiece;state.x=state.y=0;update()}
